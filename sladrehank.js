Sladrehank = function () {
	...
	const domain = encodeURI(window.location.hostname),
		autoBlockerScript = document.querySelector(
			"script[src*='usercentrics'][src*='autoblocker.js']"
		),
		autoBlockerSrc =
	...
	this.allowAll =
		"_ucCmp" in window
			? _ucCmp.cmpController.consent.status === "ALL_ACCEPTED"
				? !0
				: !1
			: !1;
	...
	this.help = function () {
		const methods = {
			...
			properties = {
				"Sladrehank.allowAll": {
					description: "Returns whether all services are allowed to load.",
				},
	...
	function printElementsBlockedPriorConsent() {
		if (blocked.length === 0 && !this.allowAll)
			issues.push("No services appear to be marked up.");
		if (blocked.length > 0)
			list(
				blocked,
	...
	(function checkLoadingSequence() {
		...
		xhttp.onreadystatechange = function () {
			...
			var cmpRegEx =
					/([a-z]{3}\.){1,2}usercentrics(-sandbox)?\.eu\/([\d.a-z-]+\/){1,2}(bundle(_legacy)?|loader|main)\.js/,
				loadedBefore = 0,
			...
			for (let i = 0; i < scripts.length; i++) {
				if (autoBlockerSrc.test(scripts[i])) abIndex = i;
				if (cmpRegEx.test(scripts[i])) ucCmpInSource = !0;
				if (!/\.usercentrics(-sandbox)\.eu\//.test(scripts[i]))
					if (i < abIndex || abIndex === -1) loadedBefore++;
			}
			...
			if (autoBlockingEnabled && isOneCMP() && loadedBefore > 0 && abIndex > -1)
				issues.push(
